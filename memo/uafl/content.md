...
- コードカバレッジはファザーの有効性を示す二次的な尺度として意味がある
- typestate 解析によって特定されたコードカバレッジのみを計算する
  - UAF 脆弱性の発見を助けるため
  - カバレッジは生成されたテストケースが UAF にどれだけ近いかを測定できる
  - たぶん全部のコードカバレッジを測定するわけじゃないということ

<img src="./images/figure5.png" class="img-60" />

- 初期段階ではすべてのファザーが同様のコードカバレッジの成長を示す
- 時間が経つと, $UAFL$ と $UAFL_{NoIF}$ は他のファザーより速く成長する
- AFL, AFLFast, MOpt, Angora, QSYM の結果は互いに近い
- FairFuzz はこの5つのファザーよりも平均的に良い
  - いくつかの稀なエッジを個別にカバーできるが, 操作シーケンス全体をカバーすることは困難
- readelf-2.28 で Angora は最高のコードカバレッジ
  - しかし, Angora はここのエッジのみをカバーしシーケンスをカバーできていないため, 脆弱性を発見できなかった