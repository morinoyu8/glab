...
- デバイスドライバは事前に入力された (静的な) デバイス情報に基づいてデバイスを認識するか, 実行時にデバイスを検出する

#### 2.2.1 Static enumeration

- デバイスドライバの中には, あらかじめ定義された情報を使ってデバイスを認識するものもある
- ハードワイヤリングされているデバイスや, システムメモリマップの中で位置が変化しないデバイスの場合, 開発者はデバイスの位置をあらかじめ簡単に特定し, メモリアドレスなどの情報をドライバに埋め込むことができる
  - デバドラ作成した際の LED のレジスタを直接記述したこととか?
- このようなドライバは組み込みシステムでよく見られる
- Linux もハードウェア構成とトポロジを静的に記述する DTB (Device Tree Blob) ファイルを介して, 静的なデバイス列挙に対応している 

<br/>

#### 2.2.2 Dynamic probing

- 最近のバスアーキテクチャ (PCI, USB, ISA など) の多くはユーザが新しいデバイスをプラグインできるようになっている
- OS はバスをスキャンし, どのデバイスがバスに接続されているかを把握し, 実行時にバストポロジを更新する責任を負う
- このような動的プロービングでは, 複雑なロジックが含まれる可能性がある
  - 特定のアドレスにあるデバイスの選択
  - デバイスのベンダーID やプロダクトID の読み取り
  - システムメモリマップにおけるデバイスのアドレスの設定
  - デバイスの初期化の実行
  - デバイスの IRQ の有効化

<br/>

- 例えば, OS は図1のように, (1) PCI_ADDR の PCI 設定レジスタに, "バス", "デバイス", "機能" ID を書き込み, (2) PCI_DATA のレジスタから応答を読み出すことによって, PCI デバイスのプロービングを開始することができる
  - デバイスがバス上に存在する場合, PCI_DATA レジスタは 0以外の値に設定される
- 次に OS はデバイスのベンダーID (VID) とプロダクトID (PID) を読み出し, そのデバイスに対応するドライバを見つけ, ドライバのプロービング関数を呼び出す
- 最後に, プロービング関数はデバイス固有の設定と初期化処理を実行する
...
- 悪意のあるデバイスが接続可能な被害マシンに, 攻撃者が物理的にアクセスできると仮定する
- 悪意のあるデバイスは被害者のドライバにより適切にプローブされるかもしれない
- 攻撃者は MMIO, PIO, DMA, IRQ インターフェースを介して, 被害者デバドラに任意のデータを送信し, ドライバのソフトウェア脆弱性を悪用する可能性がある